...
public class PillarProcessor implements ChiselQuadProcessor {
    ...
    public PillarProcessor() {
        this(null);
    }

    public PillarProcessor(ResourceLocation targetSprite) {
    ...
    public List<BakedQuad> processQuad(BakedQuad quad, BlockPos pos, BlockAndTintGetter level, BlockState state) {
        ...
        if (face == null || face == Direction.UP || face == Direction.DOWN) {
            return PatternProcessor.remapToTile(quad, 0, 0, 2, 2, true, targetSprite);
        }
        ...
        return PatternProcessor.remapToTile(quad, tileX, tileY, 2, 2, true, targetSprite);
    }
}
