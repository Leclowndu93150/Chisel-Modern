...
public class BallOMossEntity extends ThrowableItemProjectile {
    ...
    private void turnToMoss(Level level, BlockPos pos) {
        ...
        if (newState == null && ChiselBlocks.COBBLESTONE != null) {
            for (DeferredBlock<?> deferredBlock : ChiselBlocks.COBBLESTONE.getAllBlocks()) {
                if (deferredBlock.get() == block) {
                    String variantName = getVariantName(deferredBlock);
                    DeferredBlock<?> mossyVariant = ChiselBlocks.COBBLESTONE_MOSSY.getBlock(variantName);
        ...
        if (newState == null && ChiselBlocks.TEMPLE != null) {
            for (DeferredBlock<?> deferredBlock : ChiselBlocks.TEMPLE.getAllBlocks()) {
                if (deferredBlock.get() == block) {
                    String variantName = getVariantName(deferredBlock);
                    DeferredBlock<?> mossyVariant = ChiselBlocks.TEMPLE_MOSSY.getBlock(variantName);
                    if (mossyVariant != null) {
                        newState = mossyVariant.get().defaultBlockState();
                    }
                    break;
